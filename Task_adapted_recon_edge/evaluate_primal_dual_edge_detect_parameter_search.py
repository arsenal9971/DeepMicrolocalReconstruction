...
with tf.name_scope('edge_detect'):
    edge_result = reference_unet(x_result, 1,
                                 ...
                                 name='edge_result')
    edge_result_prob = tf.sigmoid(edge_result)
...
with tf.name_scope('loss'):
    ...
    loss = const * loss_tomography + loss_seg
...
for const_val in c_values:

    base_name = 'learned_primal_dual_edge_detect_parameter_search'
    name = base_name + '/' + str(const_val)

    saver.restore(sess,
                  adler.tensorflow.util.default_checkpoint_path(name))

    loss_result, x_result_result, edge_result_prob_result, loss_tomography_result, loss_seg_result = sess.run([loss, x_result, edge_result_prob, loss_tomography, loss_seg],
    ...
    print('const= {}, loss_tomo = {}, loss_segment = {}'.format(const_val, loss_tomography_result, loss_seg_result))

    import matplotlib.pyplot as plt
    operator.range.element(y_arr_validate.squeeze()).show(saveto='sinogram.pdf')
    space.element(x_true_arr_validate.squeeze()).show(clim=[0.9, 1.1], saveto='true.png')
    space.element(edge_arr_validate.squeeze()).show(clim=[0.0, 1.0], saveto='segment_true.png')
    space.element(x_result_result.squeeze()).show(clim=[0.9, 1.1], saveto='result_{}.png'.format(const_val))
    space.element(edge_result_prob_result.squeeze()).show(clim=[0.0, 1.0], saveto='segment_{}.png'.format(const_val))
    plt.close('all')
