...
with tf.name_scope('summaries'):
    ...
    tf.summary.image('x_result_window', tf.maximum(0.9, tf.minimum(x_result, 1.1)) - 0.9)
    tf.summary.image('x_true', x_true)
    tf.summary.image('x_true_window', tf.maximum(0.9, tf.minimum(x_true, 1.1)) - 0.9)
...
for i in range(0, maximum_steps):
    ...
    if i>0 and i%50 == 0:
        lt, merged_summary_result, global_step_result = sess.run([loss_tomography, merged_summary, global_step],
        ...
        print('iter={}, loss_tomography={}'.format(global_step_result, lt))
