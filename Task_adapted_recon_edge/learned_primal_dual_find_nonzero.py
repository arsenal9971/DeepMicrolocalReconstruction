...
def generate_data(validation=False):
    ...
    edge_arr = np.empty((n_generate, space.shape[0], space.shape[1], 1), dtype='float32')
    ...
    for i in range(n_generate):
        ...
        edge_arr[i, ..., 0] = np.greater(np.abs(phantom), 1e-5)

    return y_arr, x_true_arr, edge_arr
...
with tf.name_scope('loss'):
    ...
    loss = loss_tomography + 0.001 * loss_seg
...
with tf.name_scope('summaries'):
    ...
    tf.summary.image('x_edge_result', tf.sigmoid(edge_result)[..., 0:1])
